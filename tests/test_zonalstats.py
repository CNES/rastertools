...
def test_zonalstats_category(compare, save_gen_as_ref):
    ...
    inputfile = data_path + "DSM_PHR_Dunkerque.tif"
    outformat = "GeoJSON"
    statistics = "min max mean std count range sum".split()
    # Category inputs
    categoryfile = data_path + "OSO_2017_classification_dep59.shp"
    categorydic = data_path + "OSO_nomenclature_2017.json"

    tool = Zonalstats(statistics, area=True)
    tool.with_output(out_path, output_format=outformat)
    tool.with_geometries(geometries=data_path + "COMMUNE_59xxx.geojson")
    ...
    if compare:
        match, mismatch, err = utils4test.cmpfiles(out_path, __refdir, gen_files, tolerance= 1e-8)
        assert len(match) == 1
        assert len(mismatch) == 0
        assert len(err) == 0
    elif save_gen_as_ref:
    ...
    inputfile = data_path + "DSM_PHR_Dunkerque.tif"
    outformat = "GeoJSON"
    categoryfile = data_path + "OCS_2017_CESBIO_extract.tif"
    categorydic = data_path + "OSO_nomenclature_2017.json"
